...
def load_data(path=DATA_PATH) -> pd.DataFrame:
    """Загрузить данные из CSV файла и проверить наличие целевой переменной."""
    path = Path(path)
    if not path.exists():
...
def drop_leakage_columns(df: pd.DataFrame) -> pd.DataFrame:
    """Удалить колонки, которые могут содержать утечку данных. Config LEAKAGE_COLUMNS содержит список таких колонок."""
